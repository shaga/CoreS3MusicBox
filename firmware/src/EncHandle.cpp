...
static void IRAM_ATTR pcnt_intr_handler(void *arg)
{
  ...
  pcnt_counter_clear(EncHandle::kPCntUnit);
...
void EncHandle::begin(QueueHandle_t queue, bool to_resume)
{
  ...
  pcnt_config.counter_h_lim = kFilterValue;
  ...
  pcnt_event_enable(kPCntUnit, PCNT_EVT_THRES_0);
  pcnt_set_event_value(kPCntUnit, PCNT_EVT_THRES_0, event_count_);
...
void EncHandle::setEventCount(int16_t count)
{
  event_count_ = count;
  pcnt_counter_pause(kPCntUnit);
  pcnt_counter_clear(kPCntUnit);
